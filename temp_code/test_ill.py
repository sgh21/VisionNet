...
#     parser.add_argument('--img1', type=str, default='H:\WorkSpace\VisionNet\dataset\calib_data\light_test\Image_50000.bmp',help='第一张图像路径')
#     parser.add_argument('--img2', type=str, default='H:\WorkSpace\VisionNet\dataset\calib_data\light_test\Image_5000.bmp', help='第二张图像路径')
...
import random
import cv2
from skimage.metrics import structural_similarity as ssim  # 导入SSIM计算函数
...
def visualize_alignment_effects(img1, img2, save_path=None):
    ...
    # 计算SSIM得分
    ssim_original = ssim(img1_np, img2_np, multichannel=True, channel_axis=2)
    ssim_global_mean = ssim(img1_np, img2_global_mean_np, multichannel=True, channel_axis=2)
    ssim_global_full = ssim(img1_np, img2_global_full_np, multichannel=True, channel_axis=2)
    
    print(f"SSIM Scores:")
    print(f"Original: {ssim_original:.4f}")
    print(f"Global Mean Alignment: {ssim_global_mean:.4f}")
    print(f"Global Mean+Variance Alignment: {ssim_global_full:.4f}")
    ...
    ssim_values = [ssim_original, ssim_global_mean, ssim_global_full]
    ssim_labels = ['Original', 'Global Mean', 'Global Mean+Var']
    
    plt.bar(ssim_labels, ssim_values, color=['gray', 'blue', 'green'])
    plt.title('SSIM Scores (Higher is Better)', fontsize=14)
    plt.ylabel('SSIM Score')
    plt.ylim([0, 1])  # SSIM范围为0-1
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    ...
    if save_path:
        try:
            ...
            # 保存SSIM图表
            save_file = f"{save_path}_ssim_scores.png"
            print(f"Saving: {save_file}")
            fig_ssim.savefig(save_file, dpi=300, bbox_inches='tight')

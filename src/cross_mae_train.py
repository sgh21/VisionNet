...
def main(args):
    ...
    dataset_train = CrossMAEDataset(args, is_train=True, transform=transform_train,  use_fix_template=args.use_fix_template)
    dataset_val = CrossMAEDataset(args, is_train=False, transform=transform_val,  use_fix_template=args.use_fix_template)
...
loss_norm = [10, 5, 20] # x,y,rz的归一化权重
...
def train_one_epoch(model: torch.nn.Module,data_loader, optimizer: torch.optim.Optimizer, criterion, 
    ...
    for data_iter_step, (img1, img2, label1, label2) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            ...
            loss = criterion(pred, delta_label)
...
def validate(model: torch.nn.Module,data_loader, criterion, 
             device: torch.device, epoch, log_writer=None, args=None):
    model.eval()
    metric_logger = misc.MetricLogger(delimiter="  ")
    header = 'Test:'
